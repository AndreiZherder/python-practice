...
def main():
    with open('../data/artsy-secret.json', 'r') as f:
        secret = json.load(f)
    response = requests.post("https://api.artsy.net/api/tokens/xapp_token", data=secret)
    token = response.json()['token']

    with open('../data/dataset_24476_4.txt', 'r') as f:
        artists = []
        for artist in f:
            artist = artist.strip()
            response = requests.get(f"https://api.artsy.net/api/artists/{artist}", headers={"X-Xapp-Token": token})
    ...
    with open('../data/out.txt', 'w', encoding='utf-8') as f:
        for artist in artists:
            f.write(f'{artist["sortable_name"]}\n')
