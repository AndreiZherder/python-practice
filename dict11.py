...
# в словаре вариантов, он будет засчитывать это как правильную расстановку ударения, а если не соответствует,
# то как ошибку.
#
...
# Слова в словаре расположены в алфавитном порядке. Если есть несколько возможностей расстановки ударения
...
for _ in range(int(input())):
    word = input()
    book.add(word)
    lower_book.add(word.lower())
words = input().split()
err_count = 0
for word in words:
    if count_upper(word) != 1 or (word not in book and word.lower() in lower_book):
        err_count += 1
print(err_count)
